...
/* 인자를 스택에 올린다 */
void
argument_stack (char **argv, int argc, struct intr_frame *if_) { // if_는 인터럽트 스택 프레임 => 여기에다가 쌓는다
	...
	for (int i = argc-1; i >= 0; i--) {
		...
		각 인자에서 인자 크기(argv_len)를 읽고 (이 때 각 인자에 sentinel이 포함되어 있으니 +1 - strlen에서는 sentinel 빼고 읽음)
		그 크기만큼 rsp를 내려준다. 그 다음 빈 공간만큼 memcpy를 해 준다
		*/
	...
	while (if_->rsp % 8 != 0)
	{
		...
		*(uint8_t *) if_->rsp = 0;  // 데이터에 0 삽입 => 8바이트 저장
		// 스택 포인터가 8의 배수가 되도록 조정하고, 그 빈 공간에 0을 채운다
		// 8바이트 정렬은 x86-64 아키텍처의 스택 구조에서 성능을 높이기 위해 필요하다	
	}
	...
	for (int i = argc; i >= 0; i--)
	{
		...
		if_->rsp = if_->rsp - 8;
		if (i == argc) {	// 가장 위에는 NULL이 아닌 0을 넣어야
...
/*
1. 인자들을 스택에 복사 : 각 인자를 스택에 복사하고, 해당 주소 저장
2. 워드 정렬 : 스택 포인터를 8바이트 배수로 정렬하여 메모리 접근 효율 높이기
3. 인자 포인터 배열 작성 : 스택에 복사된 인자들의 주소를 스택에 저장
4. argc와 argv 설정 : 인자의 개수(argc)와 인자 배열의 시작 주소(argv) 스택에 저장
5. 프로그램 시작 주소 설정 : 프로그램이 시작될 때 사용할 레지스터 설정
*/
...
load (const char *file_name, struct intr_frame *if_) {  //if_는 프로그램 실행 시 사용할 초기 인터럽트 프레임 
	...
	/* --- Project 2: Command_line_parsing ---*/
	/* 위에서 파싱한 인자(argument)들을 스택에 올리기
		- 지금 arg_list은 위에서 parsing한 상태로  올라가있다.
